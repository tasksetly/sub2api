...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		...
		c.TLSFingerprintProfile, c.Ticket, c.TicketMessage, c.UsageCleanupTask,
...
// QuerySender queries the sender edge of a TicketMessage.
func (c *TicketMessageClient) QuerySender(_m *TicketMessage) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		...
		Ticket, TicketMessage, UsageCleanupTask, UsageLog, User, UserAllowedGroup,

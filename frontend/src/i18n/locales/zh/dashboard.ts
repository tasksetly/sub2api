export default {
  ...
  keys: {
    ...
    test: {
      ...
      testModel: '模型调用测试',
      noGroup: '此密钥尚未分配分组，无法测试连接。',
      keyUnavailable: '只有状态为启用的密钥才能测试连接。',
      loadModelsFailed: '加载可用模型失败',
      noModels: '当前分组没有可用的文本模型。',
      httpError: 'API 请求失败（HTTP {status}）',
      noResponseBody: 'API 未返回响应内容。',
      responseFailed: 'Responses API 返回失败状态。',
      invalidStream: 'API 返回了无效的流式响应。',
      streamEndedUnexpectedly: 'API 流式响应在完成前意外结束。',
      unknownError: '未知错误',
      starting: '开始测试密钥：{name}',
      groupLabel: '分组：{group}',
      connected: '已连接到 API 网关',
      usingModel: '使用模型：{model}',
      sending: '正在发送测试消息...',
      response: '响应：',
      gatewayVerified: '已通过 {endpoint} 验证'
    },
    useKeyModal: {
      title: '使用 API 密钥',

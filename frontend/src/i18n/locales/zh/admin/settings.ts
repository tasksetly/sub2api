export default {
    settings: {
      ...
      ticketStorage: {
        title: '工单图片存储',
        description: 'S3 兼容对象存储，支持 Cloudflare R2、AWS S3、MinIO 与 OSS。',
        endpoint: 'Endpoint',
        region: 'Region',
        bucket: 'Bucket',
        ...
        testFailed: '对象存储连接失败',
        loadFailed: '加载工单存储配置失败',
        saveFailed: '保存工单存储配置失败',
      },
      features: {
        channelMonitor: {
          title: '渠道监控',
      ...
      scheduling: {
        ...
        selectionMode: '账号调度方式',
        selectionModeHint: '控制同一分组内多个可用账号的选择策略；粘性会话仍优先于此设置。',
      ...
      site: {
        ...
        brandAssetsTitle: '品牌图片',
        brandAssetsDescription: '分别配置导航栏、登录/注册页及浏览器标签页使用的图片。',
        siteLogo: '导航 Logo',
        siteContentLogo: '页面内容 Logo',
        siteFavicon: '网站图标（Favicon）',
        uploadImage: '上传图片',
        remove: '移除',
        logoHint: 'PNG、JPG 或 SVG 格式，最大 1MB。用于控制台侧边栏、首页和其他导航栏；留空则使用默认 Logo。',
        contentLogoHint: 'PNG、JPG 或 SVG 格式，最大 1MB。用于登录/注册页和简洁首页的主视觉；留空则回退到导航 Logo。',
        faviconHint: '单独上传浏览器标签页使用的 SVG、PNG 或 JPG 图标，最大 1MB。留空则兼容使用导航 Logo。',
        ...
        redeemStoreHtml: '兑换码商店提示',
        redeemStoreHtmlPlaceholder:
          '<p>还没有兑换码？<a href="https://store.example.com">前往商店购买</a></p>',
        redeemStoreHtmlHint:
          '显示在用户兑换页输入框上方。支持常用文本 HTML 和 http(s) 超链接；链接会在新窗口打开。留空则不显示。',
        compactHome: '简洁首页',
      ...
      payment: {
        ...
        validationEasyPayCustomMethodFeeRateInvalid: '易支付自定义支付方式的手续费率须为 0 到 100 之间，且最多两位小数',
        field_apiBase: 'API 基础地址',
        field_easyPayApiBaseHint: '普通易支付填服务根地址；Epusdt 填 https://<域名>/payments/epay/v1/order/create-transaction（也可填写完整 submit.php 地址），系统会自动使用跳转收银台。',
        ...
        easypayCustomMethodsHint: '添加当前易支付服务商额外支持的支付方式。支付方式会记录到 Sub2API 订单中，上游 type 会作为易支付 type 参数提交；手续费率留空时使用全局设置，同一方式在多个启用实例中应保持一致。',
        ...
        customMethodFeeRate: '手续费率',
        customMethodFeeRatePlaceholder: '使用全局',
        customMethodRedirectToPayURL: '跳转至 pay_url',

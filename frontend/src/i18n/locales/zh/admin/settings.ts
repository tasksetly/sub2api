export default {
    settings: {
      ...
      features: {
        ticketStorage: {
          title: '工单附件存储',
          description: '配置工单图片使用的 S3 兼容对象存储，支持 Cloudflare R2、AWS S3、MinIO 等服务。',
          statusEnabled: '已启用',
          statusDisabled: '未启用',
          enabled: '启用图片附件',
          enabledHint: '启用后，用户和管理员可以在工单消息中上传图片。',
          endpoint: 'S3 Endpoint',
          endpointHint: 'AWS S3 可留空；R2、MinIO 等兼容服务需要填写完整 HTTPS 地址。',
          region: 'Region',
          bucket: 'Bucket',
          accessKeyId: 'Access Key ID',
          secretAccessKey: 'Secret Access Key',
          secretConfigured: '已配置，留空保持不变',
          secretHint: '密钥已加密保存，页面不会回显明文。',
          prefix: '对象前缀',
          forcePathStyle: '使用 Path Style',
          forcePathStyleHint: 'MinIO 通常需要开启；Cloudflare R2 通常关闭。',
          maxFileSize: '单张上限（MB）',
          maxAttachments: '每条消息上限',
          urlExpiry: '预签名链接有效期（分钟）',
          testConnection: '测试连接',
          testing: '测试中...',
          save: '保存附件配置',
          saving: '保存中...',
          saved: '工单附件存储配置已保存并生效',
          testSuccess: 'S3 连接测试成功',
          testFailed: 'S3 连接测试失败',
          loadFailed: '加载工单附件存储配置失败',
          saveFailed: '保存工单附件存储配置失败',
        },
        channelMonitor: {
          title: '渠道监控',

export default {
    ...
    upstreamProviders: {
      ...
      formUsername: '登录邮箱',
      formPassword: '登录密码',
      formPasswordHelpCreate: '用 AES-256-GCM 加密存储；token 过期后需用它重新登录。若上游有 CF 校验登不上去，可留空只填下面的 Token',
      formPasswordHelpEdit: '留空表示不修改。改密码会使已缓存的 token 立即失效',
      formPasswordPlaceholderEdit: '留空则不修改',
      formToken: '上游 Token',
      formTokenHelp:
        '上游做了 Cloudflare 校验、账号密码登不上去时填这里：从浏览器开发者工具里复制上游站点的 JWT 直接粘贴。有效期从 token 的 exp 解析，解不出按 7 天算',
      ...
      tokenNoAutoRenew: '该上游只有 Token、没有密码，过期后需要手动重新粘贴',

<template>
  <div class="flex flex-wrap items-center gap-3">
    ...
    <Select :model-value="filters.upstream" class="w-40" :options="uOpts" @update:model-value="updateUpstream" @change="$emit('change')" />
  </div>
</template>
...
const props = defineProps<{ searchQuery: string; filters: Record<string, any>; groups?: AdminGroup[]; upstreamProviders?: UpstreamProviderWithStats[] }>()
...
const updateUpstream = (value: string | number | boolean | null) => { emit('update:filters', { ...props.filters, upstream: value }) }
...
// 'any' 对应后端的 AccountListUpstreamAny：只看上游建号出来的账号，不限具体上游
const uOpts = computed(() => [
  { value: '', label: t('admin.accounts.allUpstreams') },
  { value: 'any', label: t('admin.accounts.anyUpstream') },
  ...(props.upstreamProviders || []).map(p => ({ value: String(p.id), label: p.name }))
])
</script>

...
const handleBulkProbeUpstreamBilling = async () => {
  ...
  try {
    ...
    if (patched) {
      ...
      // A just-written rate can briefly lag in a read replica or cache. Reapply
      // the successful probe result after the refresh so the row cannot regress.
      results.forEach(result => {
...
const handleProbeUpstreamBilling = async (account: Account) => {
  ...
  try {
    ...
    if (result.snapshot) {
      ...
      await refreshAccountsAfterUpstreamBillingProbe()
      // Do not let a briefly stale list response overwrite the successful rate.
      patchUpstreamBillingSnapshot(account.id, result.snapshot)
    }
  } catch (error) {

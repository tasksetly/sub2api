...
describe("admin SettingsView payment visible method controls", () => {
  ...
  it("does not render repository links in payment guidance", async () => {
    const wrapper = mountView();
    ...
    expect(wrapper.findAll("a").some((node) => node.attributes("href")?.includes("github.com/Wei-Shaw/sub2api"))).toBe(false);
  });
